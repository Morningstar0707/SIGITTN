...
export default function ModalEditarTicket({ ticket, onClose, onSave }) {
  const [titulo,      setTitulo]      = useState(ticket.title || '')
  const [modulo,      setModulo]      = useState(ticket.module || '')
  const [urgencia,    setUrgencia]    = useState(ticket.urgency || '')
  const [obs,         setObs]         = useState(ticket.observations || '')
  const [dependencia, setDependencia] = useState(ticket.dependencia || '')
  ...
  const handleSubmit = () => {
    if (!validate()) return
    onSave({ id: ticket.id, title: titulo, module: modulo, urgency: urgencia, observations: obs, dependencia, responsable })
  }
    ...
    <div className={styles.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className={styles.modal}>
        ...
        <div className={styles.modalHeader}>
          <h2 className={styles.modalTitle}>Editar ticket</h2>
