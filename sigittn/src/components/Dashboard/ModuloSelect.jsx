/**
 * ModuloSelect — combobox con búsqueda en tiempo real
 * Reemplaza el <select> estándar de módulo de origen.
 *
 * Props:
 *   modulos    — array de { id_modulo_origen, nombre_modulo_origen }
 *   value      — id seleccionado actualmente (string o number)
 *   onChange   — fn(id) llamada al seleccionar
 *   error      — bool, muestra borde rojo si true
 *   disabled   — bool
 */
...
export default function ModuloSelect({ modulos = [], value, onChange, error, disabled }) {
  ...
  // Nombre del módulo actualmente seleccionado
  const seleccionado = modulos.find(m => String(m.id_modulo_origen) === String(value))

  // Filtrar módulos por búsqueda
  const filtrados = busqueda.trim()
    ? modulos.filter(m =>
        m.nombre_modulo_origen.toLowerCase().includes(busqueda.toLowerCase())
      )
    : modulos
  ...
  useEffect(() => {
    if (resaltado >= 0 && listaRef.current) {
      const item = listaRef.current.children[resaltado]
  ...
  const seleccionar = (modulo) => {
    onChange(String(modulo.id_modulo_origen))
    setAbierto(false)
    setBusqueda('')
    setResaltado(-1)
  }
    ...
    <div ref={wrapperRef} style={{ position: 'relative' }}>
      ...
      <button
        ...
        style={{
          ...
          background: '#ffffff',
          border: `1.5px solid ${error ? '#e05c5c' : abierto ? '#1e6fc5' : '#d0dae8'}`,
          borderRadius: 9,
          textAlign: 'left',
      ...
      >
        {seleccionado ? seleccionado.nombre_modulo_origen : 'Seleccionar módulo'}

        {/* Ícono derecho: X si hay selección, chevron si no */}
        <span style={{
      ...
      {abierto && (
        <div style={{
          ...
          <div style={{
            ...
            />
            {busqueda && (
              <button type="button" onClick={() => { setBusqueda(''); setResaltado(-1); inputRef.current?.focus() }}
          ...
          >
            {filtrados.length === 0 ? (
              ...
              filtrados.map((m, i) => {
                const esSeleccionado = String(m.id_modulo_origen) === String(value)
                const esResaltado = i === resaltado
                return (
                  <li
                    key={m.id_modulo_origen}
                  ...
                  >
                    <span>{resaltarTexto(m.nombre_modulo_origen, busqueda)}</span>
                    {esSeleccionado && (
